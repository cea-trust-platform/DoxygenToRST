...
from .converters.ClassConverter import ClassConverter
...
def run(input=".", output="./rst", keeprst=False, test=False):
    ...
    def ccc(file, output_dir):

        class_converter=ClassConverter(input, output)
        return class_converter.convert(file)
    ...
    for data in headers:
        ...
        writer.write_to_file(f"{output}/{data[2]}", force=True)

        doxy_writer.add_line(f"./{data[2]}")
    
    doxy_writer.end_group("toctree")

    doxy_writer.write_to_file(f"{output}/index.rst", force=True)
...
def parse_args(argv):
    ...
    parser.add_argument('-i', '--input', default=".", help="Path to directory containing the xml generated by Doxygen") 
...
if __name__ == "__main__":
    ...
    run(input=args.input, output=args.output, keeprst=args.keeprst, test=args.test)   
